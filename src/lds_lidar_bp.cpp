...
int LdsLidar::InitLdsLidar(std::vector<std::string>& broadcast_code_strs) {
  ...
  // register data callback and connection info callback
  ...
  //    printf("No broadcast code was added to whitelist, swith to automatic connection mode! existing.\n");
  //}
  LdsLidar::DisableAutoConnectMode();
  ...
  //printf("Livox-SDK init success! existing.\n");
...
void LdsLidar::OnDeviceChange(const DeviceInfo *info, DeviceEvent type) {
  ...
  if (type == kEventConnect) {
    //QueryDeviceInformation(info->handle, DeviceInformationCb, g_lidars);
  ...
  if (p_lidar->connect_state == kConnectStateOn) {
    ...
    //SetErrorMessageCallback(info->handle, LdsLidar::LidarErrorStatusCb);
